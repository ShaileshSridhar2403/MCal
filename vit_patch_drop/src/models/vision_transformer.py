...
class Attention(nn.Module):
    ...
    def forward(self, x, zero_patch=None):
        B, N, C = x.shape
        ...
        if zero_patch is not None:
            for b in range(B):
                if torch.any(zero_patch[b]):
                    attn[b, :, :, zero_patch[b]] = -float('inf')
...
class Block(nn.Module):
    ...
    def forward(self, x, zero_patch=None):
        out = self.attn(self.norm1(x), zero_patch=zero_patch)
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, patch_mask): 
        B = x.shape[0]
        ...
        if patch_mask is not None:
            ...
            if len(patch_mask.shape) == 1: # not a separate one per batch
                ...
                zero_patch = patch_mask == -1
                dummy = torch.zeros(B, 1, C).cuda()
                x = torch.cat([x, dummy], dim=1) # add a dummy vector
                patch_mask[patch_mask==-1] = N
                patch_mask = patch_mask.unsqueeze(-1).expand(-1, -1, C) # -1s will be dummy
                x = torch.gather(x, 1, patch_mask)
        else:
            zero_patch = None

        for i, blk in enumerate(self.blocks):
            x = blk(x, zero_patch=zero_patch)
