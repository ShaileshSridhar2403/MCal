#!/usr/bin/env python3
"""
Fair Comparison with MULTIPLE random ablations per fraction — Breast Cancer (binary).
Mirrors physionet_multi_ablation.py but adapted for sklearn Breast Cancer dataset.
"""
...
for cls in sorted(y_series.unique()):
    cls_df = combined[combined["diagnosis"] == cls]
...
print(f"Breast Cancer — Train: {n_train}, Test: {n_test}, Features: {n_feat}, Classes: {NUM_CLASSES}")
...
print("\n" + "=" * 90)
print(f"RESULTS — Breast Cancer (binary) (train: {train_size}, eval: {eval_size} per fraction, concatenated)")
print("=" * 90)
...
ax.set_title(f"Fair Comparison: Breast Cancer (binary)\n"
...
plt.tight_layout()
plt.savefig(SCRIPT_DIR / "breast_cancer_multi_ablation.png", dpi=150, bbox_inches="tight")
print(f"\nSaved plot to {SCRIPT_DIR / 'breast_cancer_multi_ablation.png'}")
