#!/usr/bin/env python3
"""
Clean Breast Cancer KL Divergence Benchmark

This benchmark file only handles:
1. Loading data via breast_cancer_data_setup.py
2. Applying transforms (calibration methods)
3. Calculating KL metrics
4. Saving results

All data generation logic is in breast_cancer_data_setup.py
...
def apply_mcal_calibrator(outputs, device, kappa=4.0, max_steps=10000, **kwargs):
    """Apply MCal calibrator. (Adapted for binary breast cancer)"""
    n_fractions, n_samples, n_classes = outputs.shape
    transformed_outputs = np.zeros_like(outputs)

    # Create uniform target distribution for 2 classes (same as PhysioNet)
...
def apply_mcal_ce_calibrator(outputs_tensor, target_labels, device, max_steps=5000,
                           head_type="linear", experiment_id="breast_cancer_experiment", **kwargs):
...
def process_breast_cancer_dataset(methods=None, device="cuda", save_dir="./results", n_runs=3,
                                 n_samples=1000, n_fractions=10):
    """Process breast cancer dataset and generate KL benchmarks. (Adapted from PhysioNet pattern)"""
    ...
    print("="*60)
    print("Breast Cancer Tabular KL Divergence Benchmark")
    print("="*60)
    ...
    for run in range(n_runs):
        ...
        if 'retrain' in methods:
            retrain_predictions, retrain_labels = load_breast_cancer_data(
        ...
        if 'replace' in methods:
            replace_predictions, replace_labels = load_breast_cancer_data(
        ...
        if 'archmod' in methods:
            archmod_predictions, archmod_labels = load_breast_cancer_data(
        ...
        for method in methods:
            ...
            if method == 'baseline' or method == 'retrain':
                ...
                if method == 'mcal':
                    ...
                    method_kwargs['experiment_id'] = f'breast_cancer_run_{run}'
                elif method == 'logits_sharp':
    ...
    json_path, table_path, plot_path = save_results(aggregated_results, save_dir, "breast_cancer", n_runs)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Breast Cancer Tabular KL Divergence Benchmark")
    ...
    # Run benchmark
    results = process_breast_cancer_dataset(
